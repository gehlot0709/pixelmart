            ...
            <>
              ...
              >
                <button
                  ...
                  className={`w-full text-left px-6 py-4 rounded-2xl transition-all font-black text-sm uppercase tracking-widest ${!category ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-primary'}`}
                >
                  All Infinity Products
                </button>

                <div className="h-px bg-slate-100 dark:bg-slate-700 mx-4 my-2" />

                {categories.filter(c => !c.parent).map(parent => (
                  <div key={parent._id} className="space-y-1">
                    <button
                      onClick={() => { setCategory(parent._id); setShowCategoryDropdown(false); }}
                      className={`w-full text-left px-6 py-4 rounded-2xl transition-all font-bold ${category === parent._id ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                    >
                      {parent.name}
                    </button>
                    ...
                    <div className="ml-4 pl-4 border-l-2 border-slate-50 dark:border-slate-700 flex flex-col gap-1">
                      {categories.filter(c => c.parent === parent._id).map(sub => (
                        <button
                          key={sub._id}
                          onClick={() => { setCategory(sub._id); setShowCategoryDropdown(false); }}
