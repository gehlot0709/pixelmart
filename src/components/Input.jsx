...
const Input = ({ label, type, name, value, onChange, placeholder, icon: Icon, required = false }) => {
    ...
    return (
        <div className="mb-4">
            {label && <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-widest mb-3">{label}</label>}
            <div className="relative">
                ...
                {isPassword && (
                    <button
                        ...
                        className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-600 hover:text-primary transition-colors focus:outline-none"
                    >
